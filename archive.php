  ...
  function move_messages()
  {
    ...
    foreach ($messageset as $mbox => $uids) {
      ...
      foreach ($uids as $uid) {
		...
		// hachre: JS Api disabled, so we always land here.
		// if 'read_when_archived' is set to true, mark as read.
		if ($rcmail->config->get('read_when_archived') == true) {
			$storage->set_flag(array($uid), 'SEEN');
		}
